...
interface AutoTuneRunnerInterface
{
     ...
     *   (`spy_search_ranking_auto_tune_metric_config`, store+locale scoped, matching `search-ranking`'s own
     *   formula/isActive/shape scoping) are checked independently: a threshold, auto-update setting, or
     *   notify setting on one store — or, for a genuinely locale-scoped metric, one locale of that store —
     *   has no effect on any other.
     * - For every metric with an auto-tune threshold set (within a given store — and, for a locale-scoped
     *   metric, within a given locale of it): reads the metric's CURRENT fit quality fresh (no side
     *   effect); if it's still at or above the threshold, appends an isChange=false audit row and moves on.
     *   If it's below the threshold, computes a refit (parameters-only or program's-choice, per that
     *   metric's own configured scope), applies it through search-ranking's own facade when
