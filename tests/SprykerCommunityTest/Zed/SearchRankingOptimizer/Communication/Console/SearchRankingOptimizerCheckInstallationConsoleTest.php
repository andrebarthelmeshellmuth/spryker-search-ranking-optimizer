...
class SearchRankingOptimizerCheckInstallationConsoleTest extends Unit
{
    ...
    /**
     * A real application/command wiring, same as {@see createCommandTester()}, but with an anonymous
     * subclass overriding {@see SearchRankingOptimizerCheckInstallationConsole::getGlueApiResourceClassName()}
     * so the Glue API wiring check tests `class_exists()` against a fixture class name instead of this
     * host shop's real (generator-produced) `Generated\Api\Storefront\SearchRelevanceJudgmentsStorefrontResource`.
     */
    protected function createCommandTesterWithGlueApiResourceClassName(string $resourceClassName): CommandTester
    {
        ...
        $facadeMock->method('getAutoTuneNotificationDiagnosis')->willReturn(
            $this->createDiagnosis(isNotifyEnabledAnywhere: false, doesRoleExist: true, recipientEmails: []),
        );
